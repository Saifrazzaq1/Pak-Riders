...
class PostdataU extends StatefulWidget {
  const PostdataU({super.key});

  @override
  State<PostdataU> createState() => _PostdataUState();
}

class _PostdataUState extends State<PostdataU> {
 ...
 final storeImage =FirebaseStorage.instance.ref().child('mapshotU').child('Map$imgId');
 var task = storeImage.putFile(mapshot!) ;
 
 imgurL =await (await task.whenComplete(() => null)).ref.getDownloadURL();


  FirebaseFirestore.instance.collection('MapshotU').doc(uid).set({
  
   'mapshotu': imgurL.toString(),
